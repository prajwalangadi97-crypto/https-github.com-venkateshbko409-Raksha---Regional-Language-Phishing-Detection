  ...
  const handleAnalyzeUPI = () => {
    ...
    setTimeout(() => {
      ...
      if (isReverseScam) {
        ...
        flags.push('Unverified private VPA handle spoofing an official government utility or armed forces canteen');
      }
      if (parsed['am']) {
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-teal-500/20 text-teal-400 border border-teal-500/30">
        ...
        {/* Golden Rule Pill */}
        <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-amber-950/40 border border-amber-500/30 text-amber-300 text-xs font-mono">
          <AlertTriangle className="h-4 w-4 text-amber-400 shrink-0" />
          <span>GOLDEN RULE: NO UPI PIN NEEDED TO RECEIVE MONEY</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <div className="lg:col-span-6 flex flex-col gap-4">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5 backdrop-blur-md shadow-xl space-y-4">
            <label htmlFor="upi-payload-input" className="text-xs font-bold uppercase tracking-wider text-slate-300 block">
            ...
            <textarea
              ...
              className="w-full rounded-xl border border-slate-800 bg-slate-950 p-3 text-xs font-mono text-cyan-300 placeholder-slate-600 focus:border-cyan-400 focus:outline-none"
            />
            ...
            <div className="flex items-center justify-between gap-3">
              <button
                ...
                className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-teal-500 to-cyan-500 px-5 py-2.5 text-xs font-bold text-slate-950 shadow-md shadow-teal-500/20 hover:scale-105 transition-all"
              >
              ...
              <button
                ...
                className="text-xs text-slate-400 hover:text-white transition-all font-mono"
              >
                Clear
              </button>
            </div>
            ...
            <div className="space-y-2">
              {PRESET_UPI_PAYLOADS.map((item, idx) => (
                <button
                  ...
                  className="w-full text-left p-3 rounded-xl border border-slate-800 bg-slate-950/60 hover:bg-slate-800 hover:border-cyan-500/40 transition-all text-xs"
                >
                  <span className="font-bold text-slate-200 block">{isKn ? item.labelKn : item.label}</span>
                  <span className="font-mono text-[11px] text-cyan-400 truncate block mt-0.5">{item.payload}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="lg:col-span-6 flex flex-col gap-4">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5 backdrop-blur-md shadow-xl space-y-4">
              ...
              <div className="space-y-4 animate-fade-in">
                ...
                <div className="p-4 rounded-xl bg-rose-950/40 border border-rose-500/40 text-rose-200 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono text-xs font-bold px-2 py-0.5 rounded bg-black/50 text-rose-300">
                ...
                <div className="pt-2 flex items-center justify-between gap-3 flex-wrap">
                  {onNavigateTo && (
                    <button
                      ...
                      className="px-3.5 py-2 rounded-lg bg-rose-600 hover:bg-rose-500 text-white font-bold text-xs flex items-center gap-2 shadow-lg transition-all"
                    >
            ...
            {!isAnalyzing && !result && (
              <div className="h-44 flex flex-col items-center justify-center text-center p-4 text-xs text-slate-500 border border-dashed border-slate-800 rounded-xl">
